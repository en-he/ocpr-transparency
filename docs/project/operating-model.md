...
The current site remains a static GitHub Pages/sql.js application. A hosted query backend is not part of this operation. The live deployment was found stale at commit `32f80c0` from 2026-04-19 while `origin/main` was at `468c3c4` with the July 2 site artifacts and release asset. To address the GitHub-token push-trigger exception, the sync workflow now grants only `contents: write` and `actions: write`, then explicitly dispatches the existing `pages.yml` workflow on `main` after `git-auto-commit-action@v5` reports `changes_detected == 'true'`. This is the implemented post-auto-commit freshness contract: it requests a Pages run only when the sync actually created a data/site commit, but no deployment or freshness claim is made until it is observed remotely.
...
The current repository now defines a read-only PR/main CI baseline in `.github/workflows/ci.yml`. It runs the portability check, Python compilation check, and unittest discovery after installing the declared requirements. The scheduled/manual sync job runs the same checks immediately after dependency installation and before hydration, acquisition, database mutation, artifact or release publication, or auto-commit. This remains a pre-publication validation gate; the explicit Pages dispatch above still requires remote observation before freshness is claimed.
