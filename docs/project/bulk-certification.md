...
## Immutable Phase 1 certified corpus results
...
- 602 separately classified Phase 1 compatibility `row_hash` exclusions, each linked to the excluded source record and first-seen source record;
- 1,231,508 Phase 1 compatibility canonical bulk rows;
...
The three counts—507 parser quarantines, 521 exact source duplicates, and 602 Phase 1 compatibility exclusions—measure different things and must not be combined. The exclusions reproduce Phase 1's deliberately limited 16-hex `row_hash`; they preserve that immutable historical projection contract rather than endorsing it as the Milestone B observation or identity key. Milestone B separately retains all observations and reports 1,028 strict exclusions under full versioned canonical identity without rewriting this manifest or its reports.
...
- The append-only bulk observation/lineage schema is a separately versioned Milestone B migration. It does not retroactively alter this Milestone A certification contract or its tracked artifacts.
- The intermediate Audit Workspace and review-only normalization candidate-scoring workflow remain separate post-Phase-1 releases.
