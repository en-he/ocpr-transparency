...
- The immutable Phase 1 deployment baseline contained `1,238,597` compatibility rows: `1,231,508` with `source_type=csv` and `7,089` with `source_type=live_recovery`. Milestone B replaces the bulk projection contract with 1,232,110 retained observations, 1,231,082 canonical bulk records, 1,231,603 contributor links, and 1,028 explicit exclusions while leaving the Phase 1 certification artifacts unchanged. Supplemental live recovery remains a compatibility path; no `live_monitor` rows are present.
...
- The public product is a static browser SQLite/sql.js application with downloadable artifacts. The Milestone B browser projection is defined to carry stable projection identities and validated cancellation fields while intentionally excluding the evidence, observation, exclusion, projection, and contributor audit ledgers retained in the full database; its deployed-artifact status is recorded separately until the release gate closes. A hosted query API is a target, not the current backend.
- The deterministic reviewed normalization registry is implemented and used by ingestion/browser projection. Broader Phase 2B confidence-scored entity resolution, the document/OCR lane, and Milestone C Audit Workspace are not implemented. Scheduled live monitoring is disabled.
- Phases 0 and 1 are complete: the repository/operating baseline and the available bulk archive passed their independent, reproducible, CI/Pages, and deployed-freshness gates. The separately reviewed Phase 1→2 Milestone B data-contract migration is implemented in this checkout; its public artifact/deployment status is recorded separately from immutable Phase 1 evidence. Phase 2 live reconciliation/enrichment remains a separate target.
