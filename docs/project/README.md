...
- The public product is a static browser SQLite/sql.js application with downloadable artifacts. The deployed Milestone B browser snapshot contains `1,238,171` rows: `1,231,082` strict canonical bulk records plus `7,089` supplemental live-recovery compatibility rows. It carries stable projection identities and validated cancellation fields while intentionally excluding the evidence, observation, exclusion, projection, and contributor audit ledgers retained in the full database. A hosted query API is a target, not the current backend.
...
- Phases 0 and 1 and the separately reviewed Phase 1→2 Milestone B data-contract migration are complete. Milestone B passed its reproducibility, independent-review, CI, monthly sync/release, Pages, CDN-parity, and representative live-browser gates without rewriting immutable Phase 1 evidence. Phase 2 live reconciliation/enrichment remains a separate target.
