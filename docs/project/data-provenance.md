...
- The historical pre-Milestone-B compatibility DB contained `1,238,597` rows: `1,231,508` `csv` rows plus `7,089` `live_recovery` rows. The deployed Milestone B browser snapshot contains `1,238,171` rows: `1,231,082` strict canonical bulk records plus those `7,089` supplemental live-recovery compatibility rows. There are no `live_monitor` rows. Milestone B bulk-gate counts above do not silently claim that supplemental live-recovery evidence has been recertified under the bulk ledger.
